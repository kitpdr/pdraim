...
class CustomEventEmitter {
	...
	addListener(event: string, listener: (data: SSEEventData) => void) {
		...
		if (eventListeners.length >= this.maxListeners) {
			console.log(
				`[SSE-Emitter] Warning: Event '${event}' has exceeded the maximum number of listeners (${this.maxListeners})`
			);
		}

		eventListeners.push(listener);
		console.log('[SSE-Emitter] New listener added:', {
			event,
	...
	removeListener(event: string, listenerToRemove: (data: SSEEventData) => void) {
		...
		if (eventListeners) {
			...
			console.log('[SSE-Emitter] Listener removed:', {
				event,
				remainingListeners: newListeners.length
			});
		}
	}

...
function isRateLimited(
	...
	if (endpointType === 'sse') {
		const config = isAuthenticated
			? RATE_LIMITS.sse.authenticated
			: RATE_LIMITS.sse.unauthenticated;
		...
		if (validRequests.length >= points) {
			log.debug(
				`Rate limit exceeded for IP ${ip} on ${endpointType} endpoint (${isAuthenticated ? 'authenticated' : 'unauthenticated'})`
			);

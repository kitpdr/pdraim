...
export const userTextPreferences = table('user_text_preferences', {
	userId: t
		.text('user_id')
		.references(() => users.id)
		.primaryKey(),
...
export type _ChatRoomsValidation = Omit<ChatRoom, keyof ChatRooms> &
	Omit<ChatRooms, keyof ChatRoom>;
